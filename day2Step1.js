...
function validPasswords(passwordBank) {
  ...
  return passwordInformation.reduce((numberValid, passwordEntry) => {
    const passwordLetterCount = {};
    const characterMinimum = passwordEntry.policy[0];
    const characterMaximum = passwordEntry.policy[1];
    

    for (character of passwordEntry.password) {
      if (passwordLetterCount[character]) {
        passwordLetterCount[character] += 1;
      } else {
        passwordLetterCount[character] = 1;
      }
    }

    const letterCount = passwordLetterCount[passwordEntry.letter];
    if (letterCount >= characterMinimum && letterCount <= characterMaximum) {
      numberValid++
    }
